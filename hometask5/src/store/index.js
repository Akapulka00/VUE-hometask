...
export default createStore({
  ...
  getters: {
    goodsText: s => good => { // getters.catsByAge(0, 4)
    if (s==0){
      console.log("ok");
    }
     return good.description.slice(0, 15);
    },
    editThisCount: s => (id=0)=> { // getters.catsByAge(0, 4)
      for ( let i=0; i<s.korzina.length; i++){
        if(s.korzina[i].id==id){
          return  s.korzina[i].count;
        }
      }
    },
    getGoods: s => id => { // getters.catsByAge(0, 4)
      for ( let i=0; i<s.korzina.length; i++){
        if(s.korzina[i].id==id){
          return  s.korzina[i].count;
        }
  ...
  mutations: {
   addEdit(s,goodid){
     console.log(goodid);
     console.log(s.editGood);
    s.goods.forEach(good => {
    if (good.id === goodid) {
      if (good.count != 0){ s.editGood = good;}
      else {s.editGood =0}}
    })
    },
    addGood(s,id){
     for ( let i=0; i<s.korzina.length; i++){
      ...
      s.korzina.push({id:id, count:1,});
      console.log(s.korzina);
    },
    delGood(s,id){
      for ( let i=0; i<s.korzina.length; i++){
        if(s.korzina[i].id==id){
          if(s.korzina[i].count-1<=0){
            s.korzina[i].count--;
           s.korzina.splice(i,1);
            return;
          }
          else {
          s.korzina[i].count--;
